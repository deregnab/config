...
class Asmjs
{
	...
	{
		кбРазмер = this._РассчитатьРазмерКучи(кбРазмер);
		// HACK Firefox 63, 64: Создание ArrayBuffer ускоряет доступ к содержимому буфера в asm.js и замедляет в обычном коде.
		// https://bugzilla.mozilla.org/show_bug.cgi?id=1512491
...

{
		...
		оСегмент.AddBox('moof', () =>
		{
			...
			if (!_дорАудио.Пусто())
			{
				оСегмент.AddBox('traf', () =>
				{
					...
					// timestamp занимает 33 бита.
					оСегмент.AddFullBox('tfdt', 1, 0, 8); // unsigned 64-bit integer
					// Firefox 64: Даже промах на 1 приводит во время воспроизведения к щелчкам при наполнении буфера меньше секунды. Округление убирает промахи.
					мбМедиасегмент.setUint64(оСегмент.уКонец - 8, Math.round(_дорАудио.чВДНачала / TS_TIMESCALE * _чЧастотаДискретизации)); // basemediadecodetime
	...
	// См. м_Преобразователь.ПреобразоватьСледующийСегмент().
