...
const м_Отладка = (() =>
{
	...
	function ЗавершитьРаботуИОтправитьОтчет(сПричинаЗавершенияРаботы, буфОтправить)
	{
		if (!г_лРаботаЗавершена)
		{
			...
			// Firefox 47 x86 + Windows + XUL: При нехватке памяти (commit) может кинуть исключение со строкой "out of memory"
			// в player.js, worker.js и worker.onerror. Также может кинуть исключение с объектом QuotaExceededError во время
			// добавления или удаления потока в <video>.
...
const м_Статистика = (() =>
{
	...
	function РазобранСписокСегментов(оСписок)
	{
		...
		if (ОкноПоказано())
		{
			...
			Узел('статистика-список').textContent =
				`${кОбычныхСегментов} × ${(чДлительностьСписка / кОбычныхСегментов).toFixed(1)} = ${чДлительностьСписка.toFixed(1)} + ${кБыстрыхСегментов}`;
...
const м_Оформление = (() =>
{
	...
	function ОбновитьСтили()
	{
		...
		оСтиль.setProperty('--чНепрозрачностьОкна', Ограничить(чНепрозрачность, 0.85, 1));
	}
...
const м_Список = (() =>
{
	...
	function РазобратьСписок(лЭтоСписокВариантов, сАбсолютныйАдресСписка, сРазбираемыйСписок)
	{
		...
		for (let мсТегИлиАдрес; мсТегИлиАдрес = рвТегИлиАдрес.exec(сРазбираемыйСписок);)
		{
			...
			{
			//
			// Адрес сегмента или списка сегментов.
			//
			case undefined:
			...
			case 'EXTINF':
				...
				// UNDONE Стандарт не оговаривает положение #EXT-X-TARGETDURATION в файле.
				Проверить(nTargetDuration !== -1);
				// Пропущен адрес?
				Проверить(!оНовыйСегмент);
				оНовыйСегмент = Object.create(null);
				// Twitch: Непосредственно перед #EXT-X-ENDLIST встречался сегмент с небольшой отрицательной длительностью.
				// Большая отрицательная длительность иногда встречается при потере медиаданных.
				...
				if (Math.round(оНовыйСегмент.чДлительность) > nTargetDuration)
				{
					...
					// Twitch: При потере медиаданных иногда встречаются сегменты с аномально большой длительностью,
			...
			//
			// Нестандартные теги списка сегментов для конкретного сегмента.
			//
			case 'EXT-X-TWITCH-PREFETCH':
				...
				оНовыйСегмент = Object.create(null);
				оНовыйСегмент.чДлительность = -1;
				оНовыйСегмент.лРазрыв = лРазрыв;
				оНовыйСегмент.сАдрес = ResolveRelativeUrl(сЗначениеТега, сАбсолютныйАдресСписка);
				//моСегменты.push(оНовыйСегмент);
				лРазрыв = false;
				оНовыйСегмент = null;
				break;
			...
			//
			// Нестандартные теги списка вариантов.
			//
...
const м_Загрузчик = (() =>
{
	...
	// TODO Firefox 46, Chrome 48, Edge 15 (выход из спячки): Иногда запрос не завершается, не посылая никаких событий.
	{
		...
		Проверить(typeof сАдрес === 'string' && (сАдрес.startsWith('https://') || сАдрес.startsWith('http://') || (чНеДольше === 0 && сАдрес.startsWith(chrome.extension.getURL('')))));
	...
	// - Могут мешать другие расширения браузера (report://07273043908108_15276834036).
	{
		...
		.then(
			сРезультат =>
			{
				...
				return `${bHttp ? 'http' : 'https'}://usher.ttvnw.net/api/channel/hls/${_сКодКанала}.m3u8?allow_source=true&fast_bread=true&allow_audio_only=true&sig=${encodeURIComponent(оРезультат.sig)}&token=${encodeURIComponent(оРезультат.token)}`;
			},
			пПричина =>
			{
...
ДобавитьОбработчикИсключений(() =>
{
		...
		ПолучитьApiРасширения(oApi =>
		{
			oApi.runtime.sendMessage(
				...
				пОтвет =>
				{
					...
					{
						// Chrome 71: Не показывать ошибку в консоли если не открыто других страниц расширения.
	...
	function НачатьРаботу()
	{
		...
		// HACK Chrome 72: Событе beforeunload посылается в неактивную вкладку, которую никто не собирается закрывать.
