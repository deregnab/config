...
const ЗАПРОС_СОСТОЯНИЯ_ТРАНСЛЯЦИИ =
`query WebPlayerChannelStream($channel: String) {
...
function ОбработатьОтвет()
{
	...
	г_оЗапрос = null;
	// Вызван abort(), проблемы с сетью, проблемы на сервере, неправильно составлен запрос...
	...
	else
	{
		...
		{
			// Прислали неведомую фигню, указанного канала не существует, трансляция завершена, трансляция закодирована...
		}
...
function ПеренаправитьНаНашПроигрыватель(сКодКанала)
{
	...
	//
	// HACK Opera 52, Vivaldi 2.3.1420.4: location.replace(chrome-extension://) не замещает, а добавляет адрес в историю.
	//
...
function ВставитьНашуКнопку()
{
	if (!г_оРазобранныйАдрес.лМобильнаяВерсия)
	{
		...
		// Моя кнопка будет всегда расположена слева от кнопок Твича, независимо от порядка всавки всех этих кнопок.
		сузКудаВставлять[0].insertAdjacentHTML('afterend',
		`
